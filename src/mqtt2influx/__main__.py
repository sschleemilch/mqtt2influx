...
def mqtt_subscribe(client: mqtt.Client, config: Config) -> None:
...
def mqtt_on_connect(client: mqtt.Client, userdata: Userdata, flags: Dict[Any, Any], rc: int) -> None:
...
def mqtt_on_message(client: mqtt.Client, userdata: Userdata, msg) -> None:  # type: ignore[no-untyped-def]
    payload = round(float(msg.payload.decode("utf-8")), 1)
    logger.info(f"Got MQTT Message, topic '{msg.topic}', payload: '{payload}'")
    device, topic = userdata.config.get_device_topic(msg.topic)  # type: ignore[misc]
...
def mqtt_loop(userdata: Userdata) -> None:
...
def main(verbose: int, config: Path) -> None:
